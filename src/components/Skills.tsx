...
const Skills = () => {
  ...
  return (
    <section id="skills" className="section-padding bg-[hsl(var(--cream))] md:bg-[hsl(var(--background))]">
      <div className="container mx-auto">
              ...
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-sm text-xs sm:text-sm md:text-base transition-all duration-300 whitespace-nowrap relative z-20
                  ${activeCategory === category.id 
                    ...
                    : 'bg-white border border-[hsl(var(--blood-red))]/30 text-[hsl(var(--charcoal))] md:bg-white/5 md:text-gray-300 md:border-white/10 hover:bg-[hsl(var(--blood-red))]/10 md:hover:bg-white/10'
                  }`}
              >
                {category.name}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
          {filteredSkills.map((skill, index) => (
            <div 
              key={index} 
              className="glass-card p-4 sm:p-6 group hover:border-[hsl(var(--blood-red))]/30 transition-all duration-300"
            >
              <div className="mb-3">
                <h3 className="text-base sm:text-lg font-semibold mb-3 text-[hsl(var(--charcoal))] md:text-white">
                  {skill.name}
                </h3>
              </div>
              <div className="w-full h-2.5 bg-gray-300 md:bg-gray-800/50 rounded-sm overflow-hidden">
                <div 
        ...
        <div className="mt-12 sm:mt-16 p-6 sm:p-8 glass-card text-center border-l-4 border-[hsl(var(--blood-red))] max-w-3xl mx-auto">
          <p className="text-body text-[hsl(var(--charcoal))] md:text-gray-300 italic mb-4">
            "Mastery is not a destination. Every bug fixed, every system optimized, 
            every line refactored—this is the path of continuous improvement."
          </p>
